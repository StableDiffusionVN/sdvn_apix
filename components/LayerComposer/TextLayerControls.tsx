...
const FONT_FAMILIES = [
    'Be Vietnam Pro', 'Asimovian', 'Playwrite AU SA', 'Arial', 'Verdana', 'Times New Roman', 'Courier New', 'Georgia', 'Comic Sans MS'
];
        ...
        <div className="p-3 space-y-4">
            <div>
                ...
                <textarea
                    ...
                    onChange={(e) => onUpdate(layer.id, { text: e.target.value }, true)}
            ...
            <div>
                ...
                <select
                    ...
                    onMouseDown={beginInteraction}
                    onChange={(e) => onUpdate(layer.id, { fontFamily: e.target.value }, true)}
                    className="form-input !p-2 !text-sm"
                >
            ...
            <div className="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input
                        ...
                        onMouseDown={beginInteraction}
                        onChange={(e) => onUpdate(layer.id, { fontSize: Number(e.target.value) }, true)}
                        className="form-input !p-2 !text-sm"
                    />
                </div>
                 <div>
                    ...
                    <select
                        ...
                        onMouseDown={beginInteraction}
                        onChange={(e) => onUpdate(layer.id, { fontWeight: e.target.value }, true)}
                        className="form-input !p-2 !text-sm"
                    >
            ...
            <div className="flex items-center justify-between gap-4 pt-2">
                ...
                {/* Align Group */}
                <div className="flex items-center gap-1 bg-neutral-700/50 p-1 rounded-md">
                     <button onClick={() => { beginInteraction(); onUpdate(layer.id, { textAlign: 'left' }, true); }} className={cn("p-1.5 rounded", layer.textAlign === 'left' ? 'bg-yellow-400 text-black' : 'hover:bg-neutral-600')} title="Align Left">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M15 15H3v2h12v-2zm0-8H3v2h12V7zM3 13h18v-2H3v2zm0 8h18v-2H3v2zM3 3v2h18V3H3z"/></svg>
                    </button>
                    <button onClick={() => { beginInteraction(); onUpdate(layer.id, { textAlign: 'center' }, true); }} className={cn("p-1.5 rounded", layer.textAlign === 'center' ? 'bg-yellow-400 text-black' : 'hover:bg-neutral-600')} title="Align Center">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M7 15v2h10v-2H7zm-4 6h18v-2H3v2zm0-8h18v-2H3v2zm4-6v2h10V7H7zM3 3v2h18V3H3z"/></svg>
                    </button>
                    <button onClick={() => { beginInteraction(); onUpdate(layer.id, { textAlign: 'right' }, true); }} className={cn("p-1.5 rounded", layer.textAlign === 'right' ? 'bg-yellow-400 text-black' : 'hover:bg-neutral-600')} title="Align Right">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M3 21h18v-2H3v2zm6-4h12v-2H9v2zm-6-4h18v-2H3v2zm6-4h12V7H9v2zM3 3v2h18V3H3z"/></svg>
                    </button>
                </div>
                {/* Color Group */}
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative h-6 w-6 rounded-full border-2 border-white/20 shadow-inner">
                        <input
                            ...
                            value={layer.color}
                            onMouseDown={beginInteraction}
                            onChange={(e) => onUpdate(layer.id, { color: e.target.value }, true)}
