...
export const LayerComposerModal: React.FC<LayerComposerModalProps> = ({ isOpen, onClose }) => {
    ...
    const [canvasSettings, setCanvasSettings] = useState<CanvasSettings>({ 
        ...
        height: 1024, 
        background: '#ffffff', 
        grid: { visible: false, snap: true, size: 50, color: '#cccccc' },
    ...
    const loadCanvasStateFromJson = useCallback((jsonData: any) => {
        ...
        const defaultGridSettings = { visible: false, snap: true, size: 50, color: '#cccccc' };
    ...
    const addImagesAsLayers = (loadedImages: HTMLImageElement[]) => {
        ...
        beginInteraction();
        let currentLayers = [...layers]; let newSelectedIds: string[] = [];
        let canvasNeedsInit = layers.length === 0 && !canvasInitialized; let canvasSettingsToUpdate = { ...canvasSettings };
        if (canvasNeedsInit) {
            const firstImg = loadedImages[0];
            canvasSettingsToUpdate = { ...canvasSettings, width: firstImg.width, height: firstImg.height };
        ...
        [...loadedImages].reverse().forEach((img) => {
            ...
            const newLayer: Layer = {
                id: Math.random().toString(36).substring(2, 9), type: 'image', url: img.src,
                x: (canvasSettingsToUpdate.width - initialWidth) / 2, y: (canvasSettingsToUpdate.height - initialHeight) / 2,
                width: initialWidth, height: initialHeight, rotation: 0, opacity: 100,
                blendMode: 'source-over', isVisible: true, isLocked: false, fontWeight: 'normal', fontStyle: 'normal', textTransform: 'none',
            };
            currentLayers = [newLayer, ...currentLayers]; newSelectedIds.push(newLayer.id);
        });
        setLayers(currentLayers); setSelectedLayerIds(newSelectedIds);
        const newHistory = history.slice(0, historyIndex + 1); newHistory.push(currentLayers);
        setHistory(newHistory); setHistoryIndex(newHistory.length - 1); interactionStartHistoryState.current = null;
    };

    const handleAddImage = (url: string) => {
        const img = new Image(); img.crossOrigin = "Anonymous";
        img.onload = () => { addImagesAsLayers([img]); };
    ...
    const handleCreateNew = useCallback(() => { setCanvasSettings({ width: 2048, height: 2048, background: '#ffffff', grid: { visible: false, snap: true, size: 50, color: '#cccccc' }, guides: { enabled: true, color: '#ff4d4d' } }); setCanvasInitialized(true); }, []);
            ...
            // Path 2: Layer(s) Selected (Image Editing / Combination)
                ...
                if (selectedLayers.length === 1) {
                    ...
                    if (isSimpleImageMode) {
                        ...
                        const newLayers = results.map(({img, originalLayer}) => ({ id: Math.random().toString(36).substring(2, 9), type: 'image', url: img.src, x: originalLayer.x + originalLayer.width + 20, y: originalLayer.y, width: originalLayer.width, height: originalLayer.height, rotation: 0, opacity: 100, blendMode: 'source-over', isVisible: true, isLocked: false, fontWeight: 'normal', fontStyle: 'normal', textTransform: 'none' } as Layer));
    ...
    }, [selectedLayers, layers, beginInteraction, updateMultipleLayers, deleteSelectedLayers, duplicateSelectedLayers, handleExportSelectedLayers]);
                            ...
                            {!canvasInitialized ? (
                                ...
                                <>
                                    <LayerComposerSidebar
                                        ...
                                        onRectBorderRadiusChange={onRectBorderRadiusChange}
